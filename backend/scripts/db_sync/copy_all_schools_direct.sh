#!/bin/bash
#
# Direct script to copy all school data at once
#
# Usage: ./copy_all_schools_direct.sh
#
...
cleanup() {
  rm -f /tmp/all_schools_bulk.csv 2>/dev/null || true
  rm -f /tmp/school_chunk_* 2>/dev/null || true
  rm -f /tmp/import_school_chunk.sql 2>/dev/null || true
}
...
echo "=========================================="
echo "Copying all school data"
echo "=========================================="

echo "Step 1: Checking what needs to be copied..."
EXISTING_COUNT=$(PGPASSWORD="$TGT_PASS" psql -h "$TGT_HOST" -p "$TGT_PORT" -U "$TGT_USER" "$TGT_DB" -t -c "SELECT COUNT(id) FROM schools;")
TOTAL_COUNT=$(psql -h "$SRC_HOST" -p "$SRC_PORT" -U "$SRC_USER" "$SRC_DB" -t -c "SELECT COUNT(id) FROM schools;")
...
echo -e "\nStep 2: Exporting missing school records..."
...
echo -e "\nStep 3: Splitting into chunks for import..."
split -l 5000 /tmp/all_schools_bulk.csv /tmp/school_chunk_

echo -e "\nStep 4: Importing to target database..."
chunk_num=0
total_chunks=$(ls /tmp/school_chunk_* 2>/dev/null | wc -l)

for chunk in /tmp/school_chunk_*; do
  ((chunk_num++))
  echo "Importing chunk $chunk_num/$total_chunks..."
  
  cat > /tmp/import_school_chunk.sql <<EOF
...
EOF
  ...
  while [[ $retry_count -lt $max_retries ]]; do
    if PGPASSWORD="$TGT_PASS" psql -h "$TGT_HOST" -p "$TGT_PORT" -U "$TGT_USER" "$TGT_DB" -f /tmp/import_school_chunk.sql 2>&1; then
      echo "✓ Imported chunk $chunk_num/$total_chunks"
      success=true
      break
    else
      ((retry_count++))
      if [[ $retry_count -lt $max_retries ]]; then
        echo "⚠ Retry $retry_count/$max_retries for chunk $chunk_num (connection issue)..."
        sleep 2
      fi
    fi
  done
...
# Final verification
FINAL_COUNT=$(PGPASSWORD="$TGT_PASS" psql -h "$TGT_HOST" -p "$TGT_PORT" -U "$TGT_USER" "$TGT_DB" -t -c "SELECT COUNT(id) FROM schools;")
echo -e "\n=========================================="
echo "✓ Import complete!"
echo "Schools in production: $FINAL_COUNT"
echo "=========================================="
