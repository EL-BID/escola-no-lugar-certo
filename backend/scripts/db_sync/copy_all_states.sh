#!/bin/bash
#
# Copy hexagons for all Brazilian states to production
# This script runs copy_state.sh for each state sequentially
#
# Usage: ./copy_all_states.sh
#
...
for state_code in $STATE_CODES; do
  ...
  if ./copy_state.sh "$state_code"; then
    ((SUCCESS_STATES++))
