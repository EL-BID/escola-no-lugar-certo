#!/bin/bash
#
# Copy education data for all Brazilian states to production IN PARALLEL
# This runs multiple state copies simultaneously for much faster execution
#
# Usage: ./copy_all_education_data_parallel.sh [max_parallel_jobs]
#
...
LOG_DIR="/tmp/edu_data_copy_logs"
mkdir -p "$LOG_DIR"
...
copy_state_with_log() {
  ...
  ./copy_education_data_state.sh "$state_code" >> "$log_file" 2>&1
