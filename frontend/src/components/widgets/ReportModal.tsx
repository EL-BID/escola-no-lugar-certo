...
function OverviewMap({
  ...
  const { boundaries, overallBounds } = useMemo(() => {
    ...
    const bounds = minLat !== Infinity ? L.latLngBounds([minLat, minLng], [maxLat, maxLng]) : null;
    return { boundaries: allBoundaries, overallBounds: bounds };
  }, [hexagons]);

  if (!overallBounds || boundaries.length === 0) {
    return (
      <div className="rounded-md border bg-muted flex items-center justify-center text-muted-foreground" style={{ width, height }}>
  ...
  return (
    <div style={{ width, height }} className="rounded-md border overflow-hidden relative z-0">
      <MapContainer center={[center.lat, center.lng]} zoom={10} style={{ width: '100%', height: '100%', zIndex: 0 }}
        zoomControl={true} attributionControl={false}>
        <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
        {boundaries.map(({ boundary, value, hexId }) => (
          <Polygon key={hexId} positions={boundary}
            pathOptions={{ color: getHexColor(value, maxValue), weight: 1, fillColor: getHexColor(value, maxValue), fillOpacity: 0.6 }} />
        ))}
...
function LeafletMapImage({ lat, lon, hexId, width = 200, height = 150 }: {
  lat: number; lon: number; hexId: string; width?: string | number; height?: number;
}) {
  const hexBoundary = useMemo(() => {
    if (!hexId) return null;
    try { return h3.cellToBoundary(hexId).map(([la, ln]) => [la, ln] as [number, number]); }
    catch { return null; }
  }, [hexId]);

  if (!lat || !lon || !hexBoundary) {
    return (
      <div className="rounded-md border bg-muted flex items-center justify-center text-muted-foreground text-xs" style={{ width, height }}>
  ...
  return (
    <div style={{ width, height }} className="rounded-md border overflow-hidden relative z-0">
      <MapContainer center={[lat, lon]} zoom={15} style={{ width: '100%', height: '100%', zIndex: 0 }}
        zoomControl={false} attributionControl={false}>
        <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
        <Polygon positions={hexBoundary} pathOptions={{ color: '#2563eb', weight: 2, fillColor: '#3b82f6', fillOpacity: 0.4 }} />
    ...
    const fetchAddress = async () => {
      try {
        const res = await fetch(
          ...
          { signal: controller.signal, headers: { 'User-Agent': 'Escola-no-Lugar-Certo/1.0' } },
        );
      ...
      } catch { setAddress({ short: 'Endereço não disponível', city: '' }); }
...
function HexagonDetailCard({
  hexId, rank, lat, lon, netBalance, byLevel, selectedLevels,
}: {
  hexId: string; rank: number; lat: number; lon: number;
  netBalance: number; byLevel: Record<EducationLevel, number>;
    ...
    <Card className="page-break-inside-avoid mb-4">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-base">
          <span className="bg-blue-500 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold">{rank}</span>
      ...
      <CardContent>
        ...
        <div className="text-xs text-muted-foreground mt-2">ID do Hexágono: {hexId}</div>
      </CardContent>
    </Card>
  );
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ReportModal — full-screen dialog overlay with blurred backdrop
// ═══════════════════════════════════════════════════════════════════════════════
export function ReportModal() {
  ...
  // Build table params from calculator state
  ...
  const processedHexagons = useMemo(() => {
    ...
    return features
      .map((f) => {
        ...
        try { const [lat, lon] = h3.cellToLatLng(hexId); latitude = lat; longitude = lon; }
        catch { /* ignore */ }
        return {
          hexId,
          latitude,
  ...
  // Reset generating state when modal opens
  useEffect(() => {
    if (reportModalOpen) {
  ...
  const handlePrint = useCallback(() => {
    ...
    // Build filename: GDE_report_{state}_{municipality}_{date}
    const stateName = appliedState?.name?.replace(/\s+/g, '_') || 'Brasil';
    const municipalityName = appliedMunicipality?.name?.replace(/\s+/g, '_') || 'todos';
    const dateStr = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
    const fileName = `Escola_no_lugar_certo_relatorio_${stateName}_${municipalityName}_${dateStr}`;

    // Set document title (browsers use it as default PDF filename)
    const originalTitle = document.title;
    document.title = fileName;

    // Clone report content into a top-level container for clean printing
    const printContainer = document.createElement('div');
    printContainer.id = 'report-print-container';
    printContainer.innerHTML = reportBody.innerHTML;
    document.body.appendChild(printContainer);

    // Add class, print, then clean up
  ...
  const handleClose = useCallback(() => { setReportModalOpen(false); }, [setReportModalOpen]);
  ...
  const hasData = appliedState && appliedEducationLevels.length > 0 && calculatorInputs && calculatorComputed;
  const loading = isGenerating || hexagonLoading;

  return (
    <Dialog open={reportModalOpen} onOpenChange={setReportModalOpen}>
      <DialogPortal>
        {/* Blurred backdrop showing dashboard behind */}
        <DialogOverlay className="bg-black/10 backdrop-blur-xs" />

        <DialogPrimitive.Content
          className="report-modal-content fixed z-50 flex flex-col bg-white rounded-2xl shadow-2xl overflow-hidden
            top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]
            w-[min(1100px,calc(100vw-2rem))] h-[min(860px,calc(100vh-2rem))]
        ...
        >
          ...
          <DialogTitle className="sr-only">Relatório de Demanda de Salas</DialogTitle>

          {/* ── Close button — top-right corner (hidden when printing) ── */}
          <button
            onClick={handleClose}
            className="print:hidden absolute top-4 right-4 z-10 rounded-sm p-1 text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
          ...
          <div className="flex-1 overflow-y-auto" id="report-modal-body">
            {!hasData ? (
              /* Empty state */
                ...
                <Card className="max-w-md">
                  <CardContent className="pt-6 text-center">
                    <p className="text-muted-foreground mb-4">
                      {!calculatorInputs || !calculatorComputed
                        ? 'A calculadora não foi configurada. Feche o relatório e configure os parâmetros da calculadora.'
                        : 'Nenhum dado selecionado. Feche o relatório e selecione uma região.'}
                    </p>
              ...
              /* Report content */
              <div className="px-4 py-6 sm:px-6 lg:px-8 print:p-4 print:max-w-none pb-12">
                {/* Brand header — top-left, with X at top-right (Figma layout) */}
                <div className="scale-75 origin-top-left -ml-2">
                    <BrandHeader />
                </div>

                {/* Title + subtitle + print button */}
                <div className="mb-8 print:mb-4 flex items-end justify-between">
                  <div>
                    <h1 className="text-2xl font-bold mb-1">Relatório de Demanda de Salas</h1>
                    <p className="text-sm text-muted-foreground">
                      Gerado em: {new Date().toLocaleString('pt-BR')}
                    </p>
                  </div>
                  <Button onClick={handlePrint} size="sm" className="print:hidden bg-blue-600 hover:bg-blue-700 text-white shrink-0">
                ...
                {/* Region & Filters */}
                <Card className="mb-6 print:mb-4 shadow-none border print:border">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-lg">Região e Filtros Selecionados</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                      <div><span className="font-semibold">Região:</span> {regionText}</div>
                      <div><span className="font-semibold">Níveis de Ensino:</span>{' '}
                        {appliedEducationLevels.map(l => EDUCATION_LEVEL_LABELS[l]).join(', ')}</div>
                      <div><span className="font-semibold">Tamanho do hexágono:</span>{' '}
                        {resInfo?.area} ({resInfo?.analogy})</div>
                      <div><span className="font-semibold">Intervalo de salas:</span>{' '}
                        {Math.floor(filterRange[0])} - {Math.ceil(filterRange[1])}</div>
                    </div>
                  </CardContent>
                </Card>

                {/* Overview Map */}
                <Card className="mb-6 print:mb-4 shadow-none border">
                  <CardHeader className="pb-2">
                  ...
                  <CardContent>
                    ...
                    <OverviewMap hexagons={processedHexagons} maxValue={processedHexagons[0]?.totalNewClassrooms || 0} height={400} />
                ...
                {calculatorInputs && (
                  <Card className="mb-6 print:mb-4 shadow-none border page-break-inside-avoid">
                    <CardHeader className="pb-2">
                ...
                <div className="page-break-before">
                  ...
                  <div className="space-y-4">
                    {processedHexagons.slice(0, 20).map((hex) => (
                      <HexagonDetailCard key={hex.hexId} hexId={hex.hexId} rank={hex.rank}
                        lat={hex.latitude} lon={hex.longitude} netBalance={hex.netBalance}
                        byLevel={hex.byLevel} selectedLevels={appliedEducationLevels} />
                    ))}
                  </div>
